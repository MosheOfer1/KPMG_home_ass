...
extract.py
-----------
Use Azure AI Document Intelligence (v4.0 GA) to read Hebrew forms, then
use Azure OpenAI to normalize and emit structured JSON.

Features:
- Reads configuration from .env (DI + AOAI endpoints, keys, model IDs).
- Supports local PDF files and URL sources.
- Geometrically labels checkboxes (nearest line to the left of each mark).
- Sends raw text + checkbox labels to Azure OpenAI for schema mapping.
- CLI: --file / --url / --model / --hebrew / --out
"""
...
from Part_1.utils import _gather_text_lines, _gather_labeled_checkboxes, _build_system_prompt, _build_user_prompt, \
    _ensure_json
